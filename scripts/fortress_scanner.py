...
def get_default_path():
    """Detects OS and returns a valid path for downloads/testzone"""
    home = os.path.expanduser("~")

    # Android (using Pydroid/Termux)
    if os.path.exists('/storage/emulated/0/'):
        return '/storage/emulated/0/Download/'

    # iOS (using Pythonista/iPydroid)
...
if os.path.exists(model_path):
    ...
    print("Please ensure the JSON model file is in the same directory as this script.")
    sys.exit()

# --- 🎯 THE DNA ANCHOR (ElevenLabs / FakeVoice Signatures) ---
...
def run_interface():
    print("\n" + "="*40)
    print("   FORTRESS V8: AI VOICE DETECTOR")
    print("="*40)
    print(f"Detected Home Path: {get_default_path()}")

    print("\n[1] Scan Single Audio File")
    print("[2] Scan Entire Folder")
    choice = input("\nSelect an option (1 or 2): ").strip()

    path = input("Paste the full path here: ").strip().replace('"', '').replace("'", "")
    ...
    if choice == '1':
        ...
        if os.path.isdir(path):
            files = [f for f in os.listdir(path) if f.lower().endswith(('.wav', '.mp3', '.m4a', '.flac'))]
            if not files:
        ...
        print("❌ Invalid selection. Please restart and pick 1 or 2.")
